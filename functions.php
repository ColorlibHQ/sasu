 ...
 */
if ( ! function_exists( 'sasu_get_page_by_title' ) ) {
	function sasu_get_page_by_title( $title, $output = OBJECT, $post_type = 'page' ) {
		$query = new WP_Query(
			array(
				'post_type'              => $post_type,
				'title'                  => $title,
 ...
 */
if ( ! function_exists( 'sasu_modern_supports' ) ) {
	function sasu_modern_supports() {
		add_theme_support( 'responsive-embeds' );
		add_theme_support( 'align-wide' );
		add_theme_support( 'editor-styles' );
	}
	add_action( 'after_setup_theme', 'sasu_modern_supports', 20 );
}
