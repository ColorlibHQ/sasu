...
// enqueue css
function sasu_common_custom_css(){
    
    wp_enqueue_style( 'sasu-common', get_template_directory_uri().'/assets/css/dynamic.css' );
		$header_bg         		  = esc_url( get_header_image() );
		$header_bg_img 			  = !empty( $header_bg ) ? 'background-image: url('.esc_url( $header_bg ).')' : '';

		$themeColor     		  = sasu_opt( 'sasu_theme_color' ) != '#0ab6ff' ? sasu_opt('sasu_theme_color') : '';

		$buttonBorderColor     	  = sasu_opt( 'sasu_button_border_color' );
		$hoverColor     	  	  = sasu_opt( 'sasu_hover_color');

		$headerTop_bg     		  = sasu_opt( 'sasu_top_header_bg_color' );
		$headerTop_col     		  = sasu_opt( 'sasu_top_header_color' );

		$headerBg          		  = sasu_opt( 'sasu_header_bg_color');
		$menuColor          	  = sasu_opt( 'sasu_header_menu_color' );
		$menuHoverColor           = sasu_opt( 'sasu_header_menu_hover_color' ) != '#000000' ? sasu_opt('sasu_header_menu_hover_color') : $themeColor;
		$dropMenuColor            = sasu_opt( 'sasu_header_drop_menu_color' );
		$dropMenuHovColor         = sasu_opt( 'sasu_header_drop_menu_hover_color' ) != '#8d00ff' ? sasu_opt('sasu_header_drop_menu_hover_color') : $themeColor;
		$headerBtnBgColor         = sasu_opt( 'sasu_booking_btn_bg_color' ) != '#8d00ff' ? sasu_opt('sasu_booking_btn_bg_color') : '';

		$footerwbgColor     	  = sasu_opt('sasu_footer_bg_color');
		$footerwTextColor   	  = sasu_opt('sasu_footer_widget_text_color') != '#7f7f7f' ? sasu_opt('sasu_footer_widget_text_color') : '';
		$widgettitlecolor  		  = sasu_opt('sasu_footer_widget_title_color');
		$footerwanchorcolor 	  = sasu_opt('sasu_footer_widget_anchor_color') != '#7f7f7f' ? sasu_opt('sasu_footer_widget_anchor_color') : $themeColor;
		$footerwanchorhovcolor    = sasu_opt('sasu_footer_widget_anchor_hover_color') != '#0ab6ff' ? sasu_opt('sasu_footer_widget_anchor_hover_color') : $themeColor;
		
		$footerNewsDefColor    	  = sasu_opt('sasu_footer_widget_anchor_color') != '#7f7f7f' ? sasu_opt('sasu_footer_widget_anchor_color') : $themeColor;

		$fofbg					  = sasu_opt('sasu_fof_bg_color');
		$foftonecolor			  = sasu_opt('sasu_fof_textone_color');
		$fofttwocolor			  = sasu_opt('sasu_fof_texttwo_color');
		...
		$customcss ="
			...
			.footer-area .single_footer_part p, .footer-area .single_footer_part p span, .footer-area .widget_sasu_newsletter .input-group input, .footer-area .copyright_part_text p, .footer-area .footer_2 .social_icon a, .footer_part .work_hours ul li p, .footer_part .work_hours ul li p span, .footer_part .work_hours h5, .footer_part .copyright_text p
			{
				color: {$footerwTextColor}
			}

			.footer_part .single_footer_part.widget_sasu_newsletter .btn{
				border-left-color: {$footerNewsDefColor}
			}

			.footer_part .single_footer_part.widget_sasu_newsletter .btn i
			{
				border-color: {$footerNewsDefColor}
			}

			.footer_part .single_footer_part.widget_sasu_newsletter .btn, .footer_part .single_footer_part.widget_sasu_newsletter .btn i, .single_sidebar_widget .tagcloud a:hover{
			...
			.footer-area .widget_sasu_newsletter .input-group .form-control::placeholder {
				color: {$footerwTextColor};
				opacity: 1;
			}
			
			.footer-area .widget_sasu_newsletter .input-group .form-control:-ms-input-placeholder {
				color: {$footerwTextColor};
			}
			...
			.footer_part .single_footer_part.widget_sasu_newsletter form .btn
			{
			...
			.footer-area .btn, .footer_part .single_footer_part.widget_sasu_newsletter form .btn{
    ...
    wp_add_inline_style( 'sasu-common', $customcss );
    
}
add_action( 'wp_enqueue_scripts', 'sasu_common_custom_css', 50 );