...
=========================================================*/
function sasu_opt( $id = null, $default = '' ) {
...
function sasu_site_icon(){
	if ( ! has_site_icon() ) {
		$html = '';
		$icon_path = SASU_DIR_ASSETS_URI . 'img/favicon.png';
...
=========================================================*/
function sasu_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_sasu_'.$id, true );
...
/*=========================================================
	Blog Date Permalink
=========================================================*/
function sasu_blog_date_permalink(){
...
/*========================================================
	Blog Excerpt Length
========================================================*/
if ( ! function_exists( 'sasu_excerpt_length' ) ) {
	function sasu_excerpt_length( $limit = 30 ) {
...
==========================================================*/
if ( ! function_exists( 'sasu_posted_comments' ) ) {
    function sasu_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__(' No Comments','sasu');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','sasu');
            } else {
                $comments = esc_html__( ' 1 Comment','sasu' );
            }
            $comments = '<i class="ti-comments"></i> '. $comments;
        } else {
            $comments = esc_html__( ' Comments are closed', 'sasu' );
        }
...
/*===================================================
	Post embedded media
===================================================*/
function sasu_embedded_media( $type = array() ){
...
/*===================================================
	WP post link pages
====================================================*/
function sasu_link_pages(){
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'sasu' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'sasu' ) . ' </span>%',
...
/*====================================================
	Theme logo
====================================================*/
function sasu_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'sasu_logo_145x32' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="sasu logo"></a>';
	}else{
	...
	if ( ! is_page_template( 'template-builder.php' ) ) {
        ...
        <section class="hero-banner d-flex align-items-center">
            <div class="container">
				...
				<?php
				if ( is_category() ) {
					single_cat_title( __('Category: ', 'sasu') );
					...
					echo esc_html__( 'Search for: ', 'sasu' ) . get_search_query();

				} elseif ( ! ( is_404() ) && ( is_single() ) || ( is_page() ) ) {
					echo  get_the_title();

				} elseif ( is_home() ) {
					echo esc_html__( 'Blog', 'sasu' );

				} elseif ( is_404() ) {
					echo esc_html__( '404 error', 'sasu' );
...
=================================================*/
function sasu_pull_right( $id = '', $condation ){
...
/*======================================================
	Inline Background
======================================================*/
function sasu_inline_bg_img( $bgUrl ){
    $bg = '';
...
/*======================================================
	Blog Category
======================================================*/
function sasu_featured_post_cat(){
...
========================================================*/
function sasu_sidebar_opt(){

    $sidebarOpt = sasu_opt( 'sasu_blog_layout' );
    $sidebar = '1';
...
/**================================================
	Themify Icon
 =================================================*/
function sasu_themify_icon(){
    return[
        'cap'     => __('Icon Cap', 'sasu'),
        'bag'     => __('Icon Bag', 'sasu'),
        'shirt'   => __('Icon T-shirt', 'sasu'),
        'cafe'    => __('Icon Cafe', 'sasu'),
    ];
}
...
============================================================*/
function sasu_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'sasu_contact7_form_content', 10, 2 );
...
/*============================================================
	Pagination
=============================================================*/
function sasu_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'sasu' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'sasu' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
=============================================================*/
if( ! function_exists('sasu_blog_single_post_navigation') ) {
	function sasu_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'sasu_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'sasu' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'sasu' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'sasu_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
/*===================================================
 Sasu Comment Template Callback
 ====================================================*/
function sasu_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'sasu' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'sasu'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'sasu' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'sasu' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'sasu_replace_reply_link_class');
function sasu_replace_reply_link_class( $class ) {
...
===========================================================*/
function sasu_latest_blog( $post_cat = 'inspiration', $post_order = 'DESC', $excerpt_limit = 18 ){
		...
		<div class="col-lg-4 col-sm-6">
			<div class="single_blog">
				<div class="appartment_img">
					<?php
						if( has_post_thumbnail() ){
							the_post_thumbnail( 'sasu_home_blog_350x374', ['alt' => get_the_title() ] );
						}
					?>
				</div>
				<div class="single_appartment_content">
					<a href="<?php the_permalink(); ?>"><h4><?php the_title(); ?></h4></a>
					<p><?php echo sasu_excerpt_length( $excerpt_limit )?></p>
					<ul class="list-unstyled">
						<li><a href="<?php echo esc_url( sasu_blog_date_permalink() )?>"> <span class="flaticon-calendar"></span> <?php the_time('M j, Y') ?> </a></li>
						<li><a href="<?php the_permalink(); ?>">  <?php echo sasu_posted_comments();?> </a></li>
					</ul>
				</div>
			</div>
		</div>
...
/*=========================================================
    Share Button Code
===========================================================*/
function sasu_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
