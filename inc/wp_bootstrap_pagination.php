...
function sasu_pagination( $args = array() ) {
    ...
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'sasu_pagination_defaults', $defaults )
    );
    ...
    if ( $previous && (1 != $page) )
        $echo .= '<li class="previous pgn__prev"><a href="' . esc_attr( $previous ) . '" title="' . esc_html__( 'previous', 'sasu' ) . '">' . esc_html__( 'Previous', 'sasu' ) . '</a></li>';
    ...
    if ($next && ($count != $page) )
        $echo .= '<li class="next pgn__next"><a href="' . esc_attr( $next ) . '" title="' . esc_html__( 'next', 'sasu') . '">' . esc_html__( 'Next', 'sasu') . '</a></li>';
